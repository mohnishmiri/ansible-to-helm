...
Helm chart for **administration** (java-ajsc) on AKS.
...
```bash
# Dev environment
helm upgrade --install administration ./administration \
  -f ./administration/values.yaml \
  -f ./administration/values-dev.yaml \
  -n com-att-attcc-dev-merge --create-namespace

# Prod environment
helm upgrade --install administration ./administration \
  -f ./administration/values.yaml \
  -f ./administration/values-prod.yaml \
...
```bash
helm uninstall administration -n com-att-attcc-dev-merge
```
...
| Parameter | Type | Default | Description |
|-----------|------|---------|-------------|
| `image.repository` | string | `acr.azurecr.io/com-att-attcc-dev-merge/administration` | Docker image repository. |
...
| `ingress.hosts[].host` | string | `administration.example.com` | Hostname for routing. |
...
helm upgrade --install administration ./administration \
  -f ./administration/values.yaml \
  -f ./administration/values-{environment}.yaml \
  -n com-att-attcc-dev-merge
```
...
```bash
# Update dependencies
helm dependency update ./administration
...
helm upgrade --install administration ./administration \
  ...
  --set ingress.hosts[0].host=administration.prod.example.com \
  --set ingress.tls[0].secretName=prod-tls \
  --set ingress.tls[0].hosts[0]=administration.prod.example.com \
  -n com-att-attcc-dev-merge
```
...
```bash
helm template my-release ./administration \
  -f ./administration/values.yaml \
  -f ./administration/values-dev.yaml
```
