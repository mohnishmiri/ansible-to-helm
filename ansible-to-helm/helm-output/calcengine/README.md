...
Helm chart for **calcengine** (java-ajsc) on AKS.
...
```bash
# Dev environment
helm upgrade --install calcengine ./calcengine \
  -f ./calcengine/values.yaml \
  -f ./calcengine/values-dev.yaml \
  -n com-att-attcc-dev-merge --create-namespace

# Prod environment
helm upgrade --install calcengine ./calcengine \
  -f ./calcengine/values.yaml \
  -f ./calcengine/values-prod.yaml \
...
```bash
helm uninstall calcengine -n com-att-attcc-dev-merge
```
...
| Parameter | Type | Default | Description |
|-----------|------|---------|-------------|
| `image.repository` | string | `acr.azurecr.io/com-att-attcc-dev-merge/calcengine` | Docker image repository. |
...
| `ingress.hosts[].host` | string | `calcengine.example.com` | Hostname for routing. |
...
helm upgrade --install calcengine ./calcengine \
  -f ./calcengine/values.yaml \
  -f ./calcengine/values-{environment}.yaml \
  -n com-att-attcc-dev-merge
```
...
```bash
# Update dependencies
helm dependency update ./calcengine
...
helm upgrade --install calcengine ./calcengine \
  ...
  --set ingress.hosts[0].host=calcengine.prod.example.com \
  --set ingress.tls[0].secretName=prod-tls \
  --set ingress.tls[0].hosts[0]=calcengine.prod.example.com \
  -n com-att-attcc-dev-merge
```
...
```bash
helm template my-release ./calcengine \
  -f ./calcengine/values.yaml \
  -f ./calcengine/values-dev.yaml
```
